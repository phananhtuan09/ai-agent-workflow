...
- description of the doc to create and its topic
- optional doc type override: feature / flow / adr
- optional absolute wiki path override

## Codex Tool Mapping

- `Read` → shell reads to inspect existing docs and templates
- `Write` / `apply_patch` → create new doc from template
- `rg` → search for existing canonical docs to avoid duplicates
- `Bash` → run focused validation when useful
- `AskUserQuestion` → ask when doc type is ambiguous or topic boundary is unclear
...
For **ADR**: use `rg` to list existing ADRs in `<WIKI_ROOT>/06_decisions/` to determine the next sequential number.

## Step 3: Check for an Existing Canonical Doc

Use `rg` to search for the topic across existing docs in the target folder and related folders.
