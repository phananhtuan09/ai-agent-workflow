...
Ask for feature name if not provided. Read `docs/ai/planning/feature-{name}.md`.
...
3. If no implementation doc: Use `!git diff --name-only main` to find changed files
...
Use @explore with medium thoroughness to compare implementation in discovered files against planning doc.
...
**Fallback:** If @explore unavailable, manually:
