...
`docs/ai/features/designs/{feature_slug}.json` is the design plan.
...
`docs/ai/features/design-decisions/{feature_slug}.json` is the durable approval provenance consumed by `create-spec` and `review-spec`.

`docs/ai/features/design-feedback/{feature_slug}.json` is the latest change-request payload from the review surface.

`docs/ai/features/specs/{feature_slug}.md` becomes the source of truth for implementation and verification after `review-spec` passes.
