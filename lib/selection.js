...
function getCliSelectedSkills(args = process.argv.slice(2)) {
  const skills = [];
  for (let index = 0; index < args.length; index += 1) {
    if (args[index] !== "--skill") continue;
    const skillId = args[index + 1];
    if (!skillId) {
      error("Missing value for --skill. Example: --skill quality-code-check");
      process.exit(1);
    }
    skills.push(skillId);
    index += 1;
...
function getCliSelectedBundles(args = process.argv.slice(2)) {
  const bundles = [];
  for (let index = 0; index < args.length; index += 1) {
    if (args[index] !== "--bundle") continue;
    const bundleId = args[index + 1];
    if (!bundleId) {
      error("Missing value for --bundle. Example: --bundle frontend");
      process.exit(1);
    }
