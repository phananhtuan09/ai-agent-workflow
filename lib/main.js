...
const { recordInstallation, snapshotInstallation } = require("./update");
...
async function main() {
  ...
  const installSelection = {
    kitId: selectedKit.id,
    runtimeIds: selectedTools.map((tool) => tool.id),
    skillIds,
  };
  const beforeFiles = snapshotInstallation(installSelection);
  const installedPaths = [
    ...(installKit(selectedTools, skillIds) || []),
    ...installProposalContract(skillIds),
  ];
  recordInstallation({
    ...installSelection,
  }, { beforeFiles });
