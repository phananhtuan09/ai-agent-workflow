...
function syncProjectWiki() {
  step("🔄 Syncing project wiki (project-wiki)...");
  syncFolder("project-wiki");
}
...
function syncAgentsMd(stepMessage = "🔄 Syncing AGENTS.md...") {
  ...
  const destFile = resolveWorkspacePath("AGENTS.md");
  const backupPath = backupFileIfExists(destFile);
  if (backupPath) {
    success(`Backed up: AGENTS.md → ${backupPath}`);
  }

  syncFile("AGENTS.md", "AGENTS.md");

  step("🔄 Syncing AGENTS.md to global scope (~/.codex/AGENTS.md)...");
  try {
    ...
    const globalBackup = backupFileIfExists(globalAgentsMdPath);
    if (globalBackup) {
      success(`Backed up: ~/.codex/AGENTS.md → ${globalBackup}`);
    }
...
function installClaudeCode() {
  ...
  const claudeMdPath = resolveWorkspacePath(".claude/CLAUDE.md");
  const claudeMdBackup = backupFileIfExists(claudeMdPath);
  if (claudeMdBackup) {
    success(`Backed up: .claude/CLAUDE.md → ${claudeMdBackup}`);
  }
  ...
  try {
    ...
    const globalBackup = backupFileIfExists(globalClaudeMdPath);
    if (globalBackup) {
      success(`Backed up: ~/.claude/CLAUDE.md → ${globalBackup}`);
    }
  ...
  step("🔄 Setting up MCP servers (.mcp.json)...");
  const mcpJsonPath = resolveWorkspacePath(".mcp.json");
  if (existsSync(mcpJsonPath)) {
    skip("Skipping (already exists): .mcp.json");
  } else {
    syncFile(".mcp.json");
