...
function installWorkflowEvaluationKit(selectedTools, skillIds) {
  ...
  step("🔄 Syncing learning workflow standard...");
  syncFile("docs/ai/project/WORKFLOW_LEARNING_STANDARD.md");
...
function installLearningWorkflowKit(selectedTools, skillIds) {
  ...
  const installedPaths = [
    ...
    "docs/ai/learning/cases/inventory-reservation.json",
  ];
  ...
  const durableCasePath = resolveWorkspacePath(
    "docs/ai/learning/cases/inventory-reservation.json"
  );
  if (existsSync(durableCasePath)) {
    skip("Skipping active-safe durable case: docs/ai/learning/cases/inventory-reservation.json");
  } else {
    syncFile("docs/ai/learning/cases/inventory-reservation.json");
  }
