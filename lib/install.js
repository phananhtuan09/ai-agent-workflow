...
const LEARNING_CASE_FILES = [
  "docs/learning/cases/inventory-reservation.json",
  "docs/learning/cases/webhook-replay-inspector.json",
  "docs/learning/cases/expiring-secret-link.json",
  "docs/learning/cases/cron-doctor.json",
  "docs/learning/cases/api-chaos-proxy.json",
  "docs/learning/cases/background-job-retry-incident.json",
];
...
function installProposalContract(skillIds) {
  if (!skillIds.includes("proposal-designer")) return [];
  step("🔄 Installing proposal namespace contract...");
  PROPOSAL_FILES.forEach((relativePath) => {
    if (existsSync(resolveWorkspacePath(relativePath))) {
      skip(`Preserving existing project file: ${relativePath}`);
    } else {
      syncFile(relativePath);
...
function installLearningWorkflowKit(selectedTools, skillIds) {
  ...
  step("🔄 Syncing learning namespace contract...");
  syncFile("docs/learning/README.md");
