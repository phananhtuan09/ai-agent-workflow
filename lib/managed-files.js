...
const PROPOSAL_FILES = ["docs/proposals/README.md"];
...
function buildManagedFiles({ sourceRoot, kitId, runtimeIds, skillIds }) {
  ...
  if (skillIds.includes("proposal-designer")) {
    PROPOSAL_FILES.forEach((relativePath) => addSourceFile(files, sourceRoot, relativePath));
  }
  files.forEach((file) => {
...
function buildManagedFilesFromSelection({ sourceRoot, selection }) {
  const files = new Map();
  const selectedSkills = [...(selection.core?.skills || [])];
  ...
  (selection.addons || []).forEach((addon) => {
    selectedSkills.push(...(addon.skills || []));
  ...
  if (selectedSkills.includes("proposal-designer")) {
    PROPOSAL_FILES.forEach((relativePath) => addSourceFile(files, sourceRoot, relativePath));
  }
  files.forEach((file) => {
    if (!isAllowedManagedPath(file.scope, file.path)) {
